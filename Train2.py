...
def trainPASNet(train_x, train_y, eval_x, eval_y, gene_mask, pathway_mask, \
                In_Nodes, Gene_Nodes, Pathway_Nodes, Hidden_Nodes, Out_Nodes, \
                Learning_Rate, L2_Lambda, nEpochs, Dropout_Rates, optimizer="Adam"):
    net = PASNet(In_Nodes, Gene_Nodes, Pathway_Nodes, Hidden_Nodes, Out_Nodes, gene_mask, pathway_mask)
    ...
    for epoch in range(nEpochs):
        ...
        net.do_m0 = dropout_mask(Gene_Nodes, Dropout_Rates[0])
        net.do_m1 = dropout_mask(Pathway_Nodes, Dropout_Rates[1])
        net.do_m2 = dropout_mask(Hidden_Nodes, Dropout_Rates[2])
        ...
        net.sc0.weight.data = net.sc0.weight.data.mul(
            net.gene_mask)  ###force the connections between gene layer and pathway layer
        ...
        do_m0_grad = copy.deepcopy(net.sc1.weight._grad.data)
        do_m1_grad = copy.deepcopy(net.sc2.weight._grad.data)
        do_m2_grad = copy.deepcopy(net.sc3.weight._grad.data)
        do_m0_grad_mask = torch.where(do_m0_grad == 0, do_m0_grad, torch.ones_like(do_m0_grad))
        ...
        for name, param in copy_state_dict.items():
            ...
            if "sc1" in name:
                final_optimal_param_mask = torch.where(do_m0_grad_mask == 0, torch.ones_like(do_m0_grad_mask),
                                                       optimal_param_mask)
